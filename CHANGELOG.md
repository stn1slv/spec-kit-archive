...
- **Python script runtime.** The command's `scripts:` frontmatter now declares `py:` beside `sh:` and `ps:`, matching every core command. A project initialised with `--script py` previously had no reliable script path and fell through to the walk-up fallback.
- **The first-party `bug` extension's report layout.** `.specify/bugs/<slug>/{assessment,fix,test}.md` is now read alongside the feature-scoped `specs/###-feature/bugs/BUG-###.md` that third-party bugfix extensions write, under the same bounded-read discipline. Because repo-level reports are not feature-scoped, they need attribution, and exactly two channels grant it: a `**Bugfix**:` annotation in this feature's artifacts naming the report, or the report's own header naming this feature. Resemblance, dates, touched files and being-the-only-report never attribute one. `fix.md` is read only for a `## Root Cause Analysis`; `test.md` is never read at all, because both describe changes in requirement-shaped prose and are exactly the leak the bounded read exists to prevent.
- **`.extensionignore`**, excluding the test fixture, local agent settings and repository plumbing from the installed copy. Installing previously copied 53 development files, the whole sample project and one baseline per released version, into every user's `.specify/extensions/archive/`. It now copies five.
- **`category`, `effect` and `homepage` in `extension.yml`.** All three are validated optional manifest fields, and the first two were already advertised in the community catalog entry while the manifest itself stayed silent, so `specify extension info` showed nothing.

### Changed

- **Agent context files are discovered, not guessed.** Since spec-kit 0.12.0 the CLI does not touch these files at all; the opt-in `agent-context` extension owns them and records which ones it manages. Step 5.3 now reads `.specify/extensions/agent-context/agent-context-config.yml` first, writes **every** file listed in `context_files` rather than the first name that happens to exist, and honours a configured marker pair instead of assuming the `<!-- SPECKIT ... -->` default. Marker detection is also shape-based now, so a per-extension block such as `<!-- SPECKIT EXT:archive START -->` is respected without this command predicting its wording. A configured target that does not exist, or a file entirely enclosed in its markers, is skipped and named rather than created or written into. Absent that config, the previous probe of `GEMINI.md`, `AGENTS.md`, `CLAUDE.md` is unchanged.
...
- Step 0.1 now names the `--json --paths-only` key set it depends on, and handles a script that *succeeds* while reporting an empty feature directory the same way it handles a non-zero exit. On current spec-kit that mode performs no validation, so success-with-nothing is the likelier outcome. That mode also no longer persists `feature.json`, which is what makes the promise that a rejected invocation leaves the repository untouched actually true, since input rejection is evaluated after the script has already run.
...
- **`.specify/init-options.json` is deliberately not read.** Its `feature_numbering` value describes how *new* features will be named, not what is already on disk, and a project that switched modes holds both forms; the directory name is ground truth. Reading it would have required widening Allowed Sources, a boundary whose value comes from being short and complete, to buy a branch with no behavioural consequence.
- Command references stay in literal `/speckit.archive.run` form rather than moving to `__SPECKIT_COMMAND_*__` tokens. The authoring guide documents that those tokens reach skills-based agents such as Codex, ZCode and Kimi verbatim, and advises against relying on them in skill bodies; upstream PR #4116 fixes the literal path instead.
