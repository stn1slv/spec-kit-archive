...
- **Agent context files are discovered, not guessed.** Since spec-kit 0.12.0 the CLI does not touch these files at all; the opt-in `agent-context` extension owns them and records which ones it manages. Step 5.3 now reads `.specify/extensions/agent-context/agent-context-config.yml` first, writes **every** file listed in `context_files` rather than the first name that happens to exist, and honours a configured marker pair instead of assuming the `<!-- SPECKIT ... -->` default. Marker detection is also shape-based now, so a per-extension block such as `<!-- SPECKIT EXT:archive START -->` is respected without this command predicting its wording. A configured target that does not exist, or a file entirely enclosed in its markers, is skipped and named rather than created or written into. When the config names nothing — which is the state a fresh install leaves it in, not an edge case — this command now performs the *same* lookup `agent-context` performs, resolving the integration key in `.specify/init-options.json` against that extension's own `agent-context-defaults.json`. The old three-name probe of `GEMINI.md`, `AGENTS.md`, `CLAUDE.md` survives only as a last resort, and now announces itself: it covers three of the thirty-seven integrations that map knows, so on a Copilot, Cursor or Qwen project it would previously find nothing or write to the wrong anchor.
