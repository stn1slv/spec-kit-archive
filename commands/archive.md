...
**Constitution is non-negotiable.** Any feature content that conflicts with a constitution MUST rule — or leaves one of its obligations unmet, per 2.1 — is flagged as CRITICAL and raised with the user before archival completes. Non-negotiable means it is never resolved silently: the user may still accept a gap knowingly, and that answer is recorded. Do not silently override or reinterpret constitution rules.
...
2. **Unmet obligations.** Some MUST rules do not forbid a behavior, they *require a statement* from any feature that does a certain thing ("every feature that stores user data MUST state its retention rule"). A feature that does the thing and never makes the statement violates the rule by omission, and this is not a "conflict" — nothing in the feature contradicts the text, the required content is simply absent. Flag it here anyway; it is exactly as binding. Obligation-shaped wording appears in Core Principles, Architecture Standards, and Quality Gates alike.
   ...
   **Bounded so this stays a check, not a survey.** Three bounds:
   ...
   - **Where the statement may live.** An obligation is met by a statement anywhere in the feature's artifacts, **unless the rule names a location** ("MUST state its retention rule *in the spec*"), in which case that location is where it must appear — a rule's own wording is never reinterpreted (0.5). A statement already in `.specify/memory/spec.md` also satisfies the obligation, since that is the archived spec this command maintains and the statement is visible to every future reader.
...
**Always ask** if any CRITICAL constitution findings were detected in 2.1 — conflicts and unmet obligations alike — because these cannot be auto-resolved. For an unmet obligation, quote the rule, what triggered it, and the feature's own Constitution Check verdict if it made one, then ask the user how to resolve it. The options are typically: **amend the feature spec and re-run this command** (the statement then arrives through an Allowed Source on the next pass, with an honest ref), **record it as an accepted gap** and archive as-is, or **declare the obligation not triggered** because the condition is not actually met. **Never write the missing statement into main memory from the user's answer** — a Step 3 answer is not an Allowed Source, and the entry would carry a `[Source: ...]` ref that is false. Whichever answer is given, the feature's content is archived either way (2.1's third bound).

**Always ask** if any supersession candidates were detected in Step 2.4, **provided the supersession gate is open** (defined once below). Removal is destructive and requires explicit confirmation. Ask this question first if the budget is tight, and bundle the 2.1 findings into a single combined question if needed.
...
**Seeding a shared scalar field beside legacy blocks.** When you write a shared scalar field in a file that also holds legacy per-feature blocks, that field's contributing values are not only this run's: the legacy blocks hold earlier features' values for the same field. Compose **all** of them into the shared field — the legacy values and this run's — one source ref per contributing feature. Writing only this run's value publishes it as *the* current implemented state while the rest of that state sits in a block the shared section supersedes in the reader's eyes, and every later run folds into the understated field.

**This is a condition, not a one-time event.** It applies whenever a legacy block still holds an uncomposed value for a shared scalar field you are writing — whether this run creates that field or an earlier run did and left the legacy value behind. Check the legacy blocks for the field each time; a version before this rule existed could have created the shared field already.

Three rules keep it narrow:

- **Scalar fields only, judged by the value.** A field is scalar when its value is one statement filling one slot (`Project Type`, `Performance Goals`, `Constraints`, `Scale/Scope`, `Language/Version`, `Storage`). A field whose value is a **list** is not, even when it sits inside Technical Context — `**Primary Dependencies**: FastAPI 0.115, SQLAlchemy 2.0, Jinja2 3.1` is a list, and so is any legacy section of bullets (Dependencies, Routing, Configuration, Testing). List-valued content stays where it is: the reader takes the union of a shared list and a legacy list and loses nothing, whereas a scalar has one slot and cannot be unioned — the same asymmetry that already earns scalars their narrow override above. Judge by the shape of the value, not by where the field sits, and do not generalize this rule beyond scalar fields.
- **Which ref the legacy contributor gets.** The seeded field is a new entry, but its legacy contributor's provenance is only as good as the legacy block's own ref, so take that ref as the starting point and apply the Edit Rules' **Legacy refs** ladder to it: identify the field in the originating feature's plan if you can (that identification read is the sanctioned Allowed Sources exception), otherwise keep the directory-level form. Never invent an artifact name for a value you did not find in that artifact. This ladder is applied to the **new** ref on the shared field only; the legacy block's own ref is left exactly as it is, because that entry was not touched.
- **The legacy blocks are still not modified**, in either branch. When the values **compose**, the composed shared field carries both, and the legacy line stays as it is, so that value now appears twice. When they **conflict**, the conflict branch above applies to what you *write*, not to what already exists: raise the Step 3 question, seed the shared field with the value describing the current implemented state, and simply **do not carry the losing value or its ref into the shared field** — that satisfies "drop the superseded value's ref together with the value it cited", because neither was ever written there. Do not move, delete, or strike the legacy scalar line to enforce it. Name every field seeded this way in the Step 6 report alongside the mixed-layout note, saying for each whether the legacy line is now a **duplicate** (composed) or a **superseded statement still carrying its own ref** (conflict), and recommend pruning it by hand — the second kind matters more, because a reader who finds it will be reading a value the shared field contradicts.
