...
**What counts as placeholder text when seeding.** A template is written to be filled in by a person authoring **one feature**, so it carries scaffolding that a project-level document should never inherit. Remove all of it: instructional comments (`<!-- ... -->`), section annotations such as `*(mandatory)*` or `*(optional)*`, example or illustrative entries including placeholder IDs, and any section whose subject is *how to author this one feature's files* rather than the project's own content (a `### Documentation (this feature)` block is the standard example). **Keep every other section heading, empty**, even one this run will not populate: an empty heading tells the next run where its content belongs, while a deleted one silently loses that. If you are unsure whether a section is scaffolding, keep it — an empty heading is cheap and a missing one is invisible.

**If `MEMORY_DIR/spec.md` does not exist and `spec.md` is in scope**:
- If `TEMPLATES_DIR/spec-template.md` exists, copy it as the seed and **leave its sections empty**, removing template placeholder text and the per-feature header block per the rule above (see **what counts as placeholder text** below)
...
- If `TEMPLATES_DIR/plan-template.md` exists, copy it as the seed and **leave its sections empty**, removing template placeholder text and the per-feature header block per the rule above (see **what counts as placeholder text** below)
...
**When a 2.1 conflict and a 2.5 contradiction cover the same content, the conflict governs.** A single incoming statement can be both — an entity extension that redefines a same-named entity *and* contradicts a MUST rule is the common case. Do not ask about it twice: the constitution question decides it, because that answer is the more restrictive of the two and it is the one the user is already being asked. **Then close it as the contradiction flow would**, whatever the constitution answer was, because the record exists for the next run rather than for this one: keep the existing entry unchanged, do not archive the incoming definition, and write the pair to the changelog's `## Unresolved Contradictions` and to `## Outstanding Items`. A withheld item leaves no trace in the spec, so without that record nothing re-raises it except re-archiving this same feature.
...
4. Scan for references to the retired ID in `.specify/memory/spec.md` itself (cross-references such as "as specified in FR-005" survive the deletion of their target), and in `plan.md`, `constitution.md`, and the agent knowledge file. Do not rewrite them — list any dangling references in the Step 6 report.
   ...
   **Also look for prose that describes the retired behavior without naming its ID.** Removing `FR-004` does not remove a plan sentence saying "completed tasks are kept forever", and an ID scan will never find it. In the same four files, name in the report any passage that plainly restates what the retired entry required. **Report only — never rewrite one.** Judging that a passage is about the retired behavior is prose judgment and can be wrong, so the output is a pointer for the user, not an edit: quote the passage, say which retired ID it appears to echo, and recommend reviewing it. When nothing obvious turns up, say so rather than staying silent, since silence here is indistinguishable from not having looked.
...
**The feature's own `## Constitution Check` is never archived.** It is a planning-time self-assessment, which is exactly why 2.1 treats it as input rather than a verdict, and archiving it puts a claim into main memory that goes stale the moment the thing it describes changes — a run that retires a requirement leaves the paragraph still asserting the retired behavior, and nothing later corrects it. Read it in 0.5 and 2.1, quote it in a Step 3 question when it bears on the answer, and do not copy it into `plan.md`. If the seeded plan carries a `## Constitution Check` heading, leave it empty; if an older version already filled one, leave that alone too (the Edit Rules' preserve-existing-layout rule), and name it under `## Outstanding Items` if a supersession this run has made it stale.
