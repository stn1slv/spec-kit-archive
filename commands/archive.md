...
2. **More than one feature.** This check comes **before** the flag check, so a range or a second path gets the guidance below rather than a generic parse error. Reject when the input covers more than one feature. The checks key on **feature-shaped tokens** — a token starting with `specs/`, or a bare feature number or name such as `007` or `007-invoice-settings` — so ordinary prose (`double-checked?`, `billing/invoicing`, `*emphasis*`) is not caught:
...
3. **Shortlist by slug overlap.** An incoming item and an existing entry form a candidate pair when their slugs share the **object noun and the action verb stem** (for entities: the same name, or the same object noun in their field lists). Rank candidates by the number of shared slug tokens, breaking ties by the incoming item's document order. Cap the shortlist at **20 pairs per target section**; when more qualify, keep the strongest by that ranking and name each dropped pair under `## Outstanding Items` with a recommendation to re-run after resolving the backlog — a dropped pair is unexamined, and its incoming item enters as a new entry, which is exactly the accumulation this step exists to prevent.
...
**Bugs addressed:** [only the bug IDs the Step 1 audit classified **addressed** — corroborated by bugfix annotations in the feature artifacts, e.g. BUG-001, BUG-003. Never list a report on the strength of its Status claim. Omit this line when none]
```
