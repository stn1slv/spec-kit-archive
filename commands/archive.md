...
1. **Find the project's agent context file(s).** Current spec-kit does not manage these files itself: the opt-in `agent-context` extension owns them, and it records which files it manages. Resolve the targets in this order:
   ...
   **A known limit of the fallback, which the report must state whenever the fallback branch is taken.** When `agent-context` is installed but its config names nothing, that extension resolves the target from its own `agent-context-defaults.json`, keyed to the agent chosen at init — which may be a file this probe does not look for, such as `.github/copilot-instructions.md`. That defaults file is deliberately **not** an allowed source: it is a lookup table, not project content, and reading it would widen the boundary for a value the user can state directly. So on a stock config this command may find no context file, or write to an `AGENTS.md` that is not the project's real anchor. Naming the fallback in the report is what lets the user notice and set `context_file` explicitly.
